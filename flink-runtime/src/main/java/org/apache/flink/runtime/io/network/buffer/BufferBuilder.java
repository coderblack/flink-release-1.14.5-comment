...
public class BufferBuilder implements AutoCloseable {
    ...
    public int appendAndCommit(ByteBuffer source) {
        ...
        commit();  // 多易教育:  将cachedPosition更新赋值给BufferBuilder的position
        return writtenBytes;
    }
    ...
    public int append(ByteBuffer source) {
        ...
        int available = getMaxCapacity() - positionMarker.getCached(); // 多易教育:  cached()即在memorySegment中的已占用长度
        int toCopy = Math.min(needed, available);
        // 多易教育:  从已占用位置，放入source数据的toCopy长度（toCopy长度为source数据长度和可用segment长度的最小值）
        memorySegment.put(positionMarker.getCached(), source, toCopy);
        positionMarker.move(toCopy); // 多易教育:  更新position标记
        return toCopy;
    }
